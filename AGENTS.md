...
- Calendar top filter button `自分の予定のみ` scrolls the page back to the schedule form's date field after filtering.
- Calendar `+ 新しい予定` button scrolls the page back to the top of the schedule form heading (`予定の追加`).
